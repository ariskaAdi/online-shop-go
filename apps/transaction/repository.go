...
func (r repository) GetTransactionsByUserPublicId(ctx context.Context, userPublicId string) (trxs []TransactionEntity, err error) {
	query := `
		SELECT
		id, user_public_id, product_id, product_price, amount, sub_total, platform_fee, grand_total, status, created_at, updated_at, product_json
		FROM transactions
